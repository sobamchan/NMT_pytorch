...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for batch in tqdm(self.train_dataloader):
            ...
            print(src_w2i['<PAD>'])
            print(src_sents[0].data)
            print(src_lens[0])
            print(src_sents[100].data)
            print(src_lens[100])
            input()
        ...
        preds = preds.view(src_sents.size(0), max(tgt_lens.tolist()), -1)
        print(' '.join([self.tgt_i2w[i.data[0]] for i in torch.max(preds[0], 1)[1]]))
